...
public class CreateOrderUseCase {
    ...
    public Order execute(Order order) {
        ...
        for (OrderItem item : order.getItems()) {
            ...
            // Regla de Negocio: Usar siempre el precio actual del sistema, NO el del frontend
            item.setUnitPrice(productInfo.getPrice());
            
            // Calcular subtotal del ítem (método del dominio)
            item.calculateSubtotal();
            
            // Opcional: Guardar detalles del producto para responder al cliente (no se guarda en BD)
